...
一个功能丰富、安全可靠的在线盲盒游戏平台，包含用户管理、多种盲盒模式、客服系统、商城系统和完整的管理后台。采用Redis缓存优化性能，支持Apache/Nginx/PHP内置服务器多种部署方式。
...
- **Redis**: 6.0+ (性能优化必需)
- **PHP Redis扩展**: php_redis (性能优化必需)
- **Web服务器**: Apache 2.4+ / Nginx / PHP内置服务器
- **系统资源**: 2GB+ RAM, 10GB+ 存储空间

### 快速部署（Windows + XAMPP + Apache）
...
```bash
# 下载php_redis.dll (PHP 8.2 TS版本)
# 从 https://pecl.php.net/package/redis 下载

# 复制到XAMPP的ext目录
Copy-Item php_redis.dll -Destination "C:\xampp\php\ext\"

# 编辑 C:\xampp\php\php.ini，添加
...
```apache
<VirtualHost *:8080>
    ServerAdmin admin@get-red.local
    DocumentRoot "C:/Users/YourUsername/Desktop/Get-red/Get-Red"
    ServerName get-red.local
    ServerAlias www.get-red.local 192.168.1.11
    
    <Directory "C:/Users/YourUsername/Desktop/Get-red/Get-Red">
        Options Indexes FollowSymLinks
        AllowOverride All
        Require all granted
    </Directory>
    
    ErrorLog "logs/get-red-error.log"
    CustomLog "logs/get-red-access.log" common
</VirtualHost>
```
...
```
127.0.0.1 get-red.local
127.0.0.1 www.get-red.local
192.168.1.11 get-red.local
```
...
```
http://localhost:8080/Get-Red/
http://192.168.1.11:8080/Get-Red/
```
...
**注意**：PHP内置服务器不支持Redis缓存和URL重写，仅用于开发测试。
...
│   │   └── ...
│   ├── config/                   # 配置文件（6个文件）
│   │   ├── database.php         # 数据库配置（292行，含31张表说明）
│   │   ├── security.php         # 安全配置（277行）
│   │   ├── redis-cache.php      # Redis缓存类（204行）
│   │   ├── draw-cache.php       # 抽奖缓存管理（350行）
...
│
├── 测试和优化
│   ├── test-draw-performance.php     # 性能测试脚本（141行）
...
项目实现了完整的Redis缓存层，包含两个核心类：

#### RedisCache类 (server/config/redis-cache.php)

基础缓存操作类，提供：
- 单例模式连接管理
- 自动故障降级（Redis不可用时自动使用数据库）
- JSON序列化
- 键前缀管理
- 分布式锁支持
- 异步队列支持
...
- `delete($key)` - 删除缓存
- `lock($key, $ttl)` - 获取分布式锁
- `lpush($key, $value)` - 推入队列
- `brpop($key, $timeout)` - 从队列弹出（阻塞）

#### DrawCache类 (server/config/draw-cache.php)

业务级缓存管理类，专门处理抽奖系统缓存：

**缓存策略**：
- 奖品列表缓存：5分钟（prizes:lucky1等）
- 用户金币缓存：1小时（user:gold:userId）
- 库存缓存：24小时（stock:prizeId）
- 抽奖锁：5秒（lock:draw:userId）

**主要功能**：
- `getPrizes($luckyPage)` - 获取奖品列表（带缓存）
- `decrementStock($prizeId)` - 原子减少库存
- `acquireLock($userId)` - 获取分布式锁（防重复抽奖）
- `getUserGold($userId)` - 获取用户金币（带缓存）
- `pushToQueue($data)` - 推入异步队列
...
| 指标 | 无缓存 | 有缓存 | 提升倍数 |
|------|--------|--------|----------|
| 单次抽奖响应时间 | 200-500ms | 10-50ms | 5-10倍 |
| 数据库查询次数 | 3-5次 | 0-1次 | 80-90% |
| 并发能力 | 100 req/s | 1000+ req/s | 10倍 |
| 数据库压力 | 高 | 低 | 大幅降低 |

### 自动故障降级

Redis缓存系统设计了完整的故障降级机制：

```php
// 如果Redis不可用，自动降级到数据库查询
if (!$drawCache->isEnabled()) {
    // 直接查询数据库
    $prizes = loadPrizesFromDB($luckyPage);
}
```

这意味着：
- Redis未安装时，项目正常运行（只是性能较差）
- Redis连接失败时，自动使用数据库
- 无需修改任何业务代码

### 监控和调试

所有缓存操作都有日志记录，可在PHP错误日志中查看：

```
DrawCache: 从缓存获取奖品列表 (页面: lucky1, 数量: 10)
DrawCache: 库存已减少 (奖品ID: 123, 剩余: 5)
DrawCache: 抽奖锁已获取 (用户ID: 456)
```
...
项目使用Apache虚拟主机进行部署，配置文件位置：`C:\xampp\apache\conf\extra\httpd-vhosts.conf`
...
项目根目录的`.htaccess`文件配置了：

1. **URL重写规则**
   ```
   /api/users.php → /server/api/users.php
   ```
   这样可以隐藏实际的文件结构

2. **安全防护**
   - 禁止直接访问配置文件（.ini, .conf, .config）
   - 设置字符编码为UTF-8
   - 启用Gzip压缩
   - 设置静态资源缓存

3. **上传目录保护**
   - uploads/目录：禁止执行PHP，仅允许图片访问
   - images/目录：禁止执行PHP，仅允许图片访问
   - logs/目录：完全禁止访问
...
验证方法：
```bash
# 查看httpd.conf中的LoadModule
grep "LoadModule" C:\xampp\apache\conf\httpd.conf
```
...
### v2.3.0 (2026-03-10) - Redis缓存和Apache部署

新增功能：
- 完整的Redis缓存系统（RedisCache + DrawCache）
- Apache虚拟主机配置
- 分布式锁防止重复抽奖
- 异步队列支持
- 自动故障降级机制
...
性能优化：
- 单次抽奖响应时间：200-500ms → 10-50ms（5-10倍提升）
- 数据库查询减少：80-90%
- 并发能力：100 req/s → 1000+ req/s（10倍提升）

部署改进：
- 支持Apache虚拟主机部署
- 支持多种访问方式（localhost、IP地址、域名）
- 完整的Windows + XAMPP部署指南
- Redis扩展自动检测和故障降级

文档更新：
- 更新README.md（新增Redis和Apache部分）
- 更新CORS白名单（8000 → 8080）
- 完整的部署步骤说明
